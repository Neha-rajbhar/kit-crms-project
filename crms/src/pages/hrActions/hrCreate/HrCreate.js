...
function HrCreate() {
  ...
  const fetchLastHrIdCount = async () => {
    try {
      const response = await axios.get(
        "https://accessible-mewing-iguana.glitch.me/generate-hrid"
      );
  ...
  const generateHrId = async () => {
    try {
      ...
      const newUserId = `VIBRANTHR${newUserIdCount}`;
      setHrId(newUserId);
  ...
  const pass = (newUserId) => {
    if (firstName && personalPhone) {
      const newPass =
        ...
        personalPhone.substring(6) +
        newUserId.substring(9);
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const response = await axios.post(
          "https://accessible-mewing-iguana.glitch.me/submit-hr",
          {
            hrId,
            firstName,
            lastName,
